...
const BENEFITS = [
  { Icon: ChatIcon, title: "Accès au Slack Nomad'Hub", body: "Des dizaines de canaux thématiques pour échanger, s'entraider et rester connecté au quotidien." },
  { Icon: ProfileIcon, title: "Networking & opportunités", body: "Rencontrez d'autres professionnels du remote, échangez sur vos projets, trouvez des collaborations." },
  { Icon: GrowthIcon, title: "Retours d'expérience & ressources", body: "Des membres partagent leurs retours, des outils et bonnes pratiques pour le travail à distance." },
  { Icon: ThumbsUpIcon, title: "Accès prioritaire aux workations", body: "Les membres abonnés accèdent en priorité aux places sur les workations les plus demandées." },
];
      ...
      <div className="mx-auto max-w-site px-6 py-14 md:px-12 md:py-[72px]">
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {BENEFITS.map(({ Icon, title, body }) => (
            <div key={title} className="flex flex-col gap-3 rounded-[22px] bg-beige p-6">
              <div className="flex h-11 w-11 items-center justify-center rounded-full border-[1.5px] border-brown/25">
                <Icon width={20} height={20} className="text-brown" strokeWidth={1.6} />
              </div>
              <h3 className="font-serif text-base font-bold text-brown">{title}</h3>
              <p className="text-[13.5px] leading-[1.55] text-brown/70">{body}</p>
            </div>
          ))}
        </div>
      </div>
